...
class Label(db.Model):
    ...
    records = db.relationship('Record', backref='label', lazy=True)
...
class Series(db.Model):
    ...
    uid = db.Column(db.String(5), index=True)
    min_duration = db.Column(db.Numeric(
        precision=10, scale=7, asdecimal=False))
    max_duration = db.Column(db.Numeric(
        precision=10, scale=7, asdecimal=False))
